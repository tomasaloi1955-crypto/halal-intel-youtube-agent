...
# Гарантии «без людей» генератор дать не может, поэтому картинку к посту мы либо берём
# готовую (обложка своего же ролика), либо РИСУЕМ САМИ — post_card.py, главная мысль поста
# на фоне с геометрическим узором. Там нет нейросети, а значит нет и случайностей.
...
def _pick_image(content, text):
    """Картинка к посту, в порядке приоритета:
      1. обложка сегодняшнего ролика — она про ту же тему и уже одобрена;
      2. фирменная карточка, нарисованная нами же (post_card.py) — главная мысль
         поста на фоне с геометрическим узором.
    Ни один из источников не может подсунуть человека в кадр — в этом и смысл
    (см. комментарий «AI-КАРТИНКИ ОТКЛЮЧЕНЫ» выше)."""
    url = _youtube_thumb_url((content or {}).get("_video_id"))
    if url:
        return url
    from post_card import card_url_for_post
    return card_url_for_post(text)
